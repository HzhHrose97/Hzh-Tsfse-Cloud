...
# 指定 JAR 文件的位置
JAR_FILE="/root/tsfse/hzh-centre/hzhCentre.jar"

# 检查是否提供了配置文件参数，如果未提供，则使用默认配置文件路径
if [ $# -eq 0 ]; then
  CONFIG_FILE="/root/tsfse/hzh-centre/config/application.yml"
  LOGBACK_CONFIG="/root/tsfse/hzh-centre/config/logback-spring.xml"
else
  CONFIG_FILE="$1"
  LOGBACK_CONFIG="$2"
fi
