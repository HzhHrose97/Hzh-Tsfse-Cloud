...
if [ $# -eq 0 ]; then
  CONFIG_FILE="/root/tsfse/hzh-gateway/config/application.yml"
...
# 指定 JVM 参数
JVM_OPTS="-Dname=hzhGateway -Duser.timezone=Asia/Shanghai -Xms512M -Xmx512M -XX:+HeapDumpOnOutOfMemoryError -XX:+PrintGCDetails -XX:NewRatio=1 -XX:SurvivorRatio=30 -XX:+UseParallelGC -XX:+UseParallelOldGC -Dspring.profiles.active=pro -Dserver.port=18083"
