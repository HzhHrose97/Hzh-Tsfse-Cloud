...
//            }
    ...
    @Override
    public MyResult sendEmailCode(String email) {
        try {
            ...
            if (!isEmailTrue){
                //邮箱格式不正确
                return MyResult.FAILED("邮箱格式不正确，请检查邮箱格式");
            }
            ...
            if (selectOne != null){
                return MyResult.FAILED("该邮箱已经注册");
            }
            ...
            if (!StringUtils.isEmpty(ipKeyTimes)){
                ...
                if (i > 100){
                    return MyResult.FAILED("请不要通过此ip"+remoteAddr+"频繁发送");
                }else {
                    i++;
            ...
            if (!StringUtils.isEmpty(emailKeyTimes)){
                ...
                if (i > 100){
                    return MyResult.FAILED("请不要通过此邮箱账号"+ email + "频繁发送");
                }else {
                    i++;
                    //2  时间数量   TimeUnit.HOURS 时间单位
            ...
            return MyResult.SUCCESS("邮箱验证发送成功");
        }catch (Exception e){
            log.error(" error",e);
    ...
    @Override
    public MyResult updatePasswordByuserSelf(RegisterDTO registerDTO) {
        try {
            String date = DateUtils.getCurrent(DateUtils.ticketPattern);
            //判断验证码是否正确
            String codeRedisKey = redisKeyUtil.mkRegisterCodeRedisKey(registerDTO.getEmail());
            ...
            if (!isEmailTrue){
                //邮箱格式不正确
                return MyResult.FAILED("邮箱格式不正确");
            }
            //对手机号进行判断
            boolean isPhone = FormatCheckUtils.isMobile(registerDTO.getPhonenumber());
            if (!isPhone ){
                return MyResult.FAILED("手机号码格式不正确");
            }
            ...
            if (hasHzhUser != null){
                ...
                if (uodateRow == 1 ){
                    return MyResult.SUCCESS("密码修改成功");
                } else {
                return MyResult.FAILED("密码修改失败");
            }
            }else {
                return MyResult.FAILED("该用户未注册");
            }
    ...
    @Override
    public MyResult sendEmailCodeWhenUpdatePassword(String email) {
        try {
            String date = DateUtils.getCurrent(DateUtils.ticketPattern);
